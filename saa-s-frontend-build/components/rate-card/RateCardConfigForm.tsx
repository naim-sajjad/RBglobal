...
function defaultRates(
  ...
  return {
    ...
    distance_bands: [
      { distance_from: 0, distance_to: null, agency_rate: 0, driver_rate: 0 },
    ],
    additional_charges: [],
  };
}
...
export function RateCardConfigForm({
  ...
  const [measurementUnit, setMeasurementUnit] = useState<'miles' | 'km'>(
    (initialData?.rates?.measurement_unit as 'miles' | 'km') ??
      employerMeasurementUnit,
  );
  const [currency, setCurrency] = useState(
    initialData?.rates?.currency ?? employerCurrency,
  );
  ...
  >(
    initialData?.rates?.additional_charges?.length
      ? initialData.rates.additional_charges.map((c) => ({
          ...
          driver_rates_by_class: c.driver_rates_by_class
  ...
  const [minTripPayByClass, setMinTripPayByClass] = useState<
  ...
  const loadFromRateCard = (card: RateCardType) => {
    ...
    if (card.rates) {
      ...
      if (r.minimum_trip_pay_agency != null)
        setMinTripPayAgency(String(r.minimum_trip_pay_agency));
      if (r.minimum_trip_pay_driver != null)
        setMinTripPayDriver(String(r.minimum_trip_pay_driver));
      if (r.minimum_trip_pay_driver_by_class) {
        setMinTripPayByClass((prev) => {
          const next = { ...prev };
          Object.entries(r.minimum_trip_pay_driver_by_class!).forEach(
            ([code, val]) => {
              next[code] = String(val);
            },
          );
  ...
  const buildPayload = (): RateCardFormData => {
    ...
    driverClasses.forEach((dc) => {
      ...
      if (v !== undefined && v !== '')
        driverByClassPayload[dc.code] = Number(v) || 0;
    });
    return {
      name,
      ...
      rates: {
        measurement_unit: measurementUnit,
        currency,
        minimum_trip_pay_agency:
          minTripPayAgency === '' ? 0 : Number(minTripPayAgency),
        minimum_trip_pay_driver:
          minTripPayDriver === '' ? 0 : Number(minTripPayDriver),
        minimum_trip_pay_driver_by_class:
          Object.keys(driverByClassPayload).length > 0
            ? driverByClassPayload
            : undefined,
        distance_bands: distanceBands.map((b) => {
          ...
          if (
            b.driver_rates_by_class &&
            Object.keys(b.driver_rates_by_class).length > 0
          ) {
        ...
        additional_charges: additionalCharges.map((c) => {
          ...
          if (
            c.driver_rates_by_class &&
            Object.keys(c.driver_rates_by_class).length > 0
          ) {
  ...
  const addBand = () => {
    ...
    driverClasses.forEach((dc) => {
      driver_rates_by_class[dc.code] = 0;
    });
    setDistanceBands([
      ...distanceBands,
      {
    ...
    setDistanceBands(
      distanceBands.map((b, i) => {
        ...
        if (
          ...
          return {
            ...b,
            driver_rates_by_class: value as Record<string, number>,
          };
        }
  ...
  const updateBandDriverRateForClass = (
    bandIndex: number,
    classCode: string,
    value: number,
  ) => {
  ...
  const updateChargeDriverRateForClass = (
    chargeIndex: number,
    classCode: string,
    value: number,
  ) => {
    ...
    <div className='space-y-8'>
          ...
          <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
            <div className='space-y-2'>
              ...
              <Input
                value={employerName}
                readOnly
                className='text-white bg-slate-700/50 border-slate-600 text-slate-300'
              />
            </div>
            <div className='space-y-2'>
            ...
            {driverClasses.length > 0 ? (
              <div className='md:col-span-2 space-y-2'>
                ...
                <p className='text-slate-400 text-sm'>
                  Agency rate and per-driver-class minimum pay
                </p>
                      ...
                      <TableRow className='border-slate-700'>
                        ...
                        <TableCell>
                          <Input
                            ...
                            onChange={(e) =>
                              setMinTripPayAgency(e.target.value)
                            }
                            className='text-white bg-slate-700 border-slate-600 text-white h-9'
                          />
                        </TableCell>
                        {driverClasses.map((dc) => (
                          <TableCell key={dc.id}>
      ...
      <Card className='bg-slate-800 border-slate-700'>
        <CardHeader className='flex flex-row items-center justify-between space-y-0'>
          <div>
            ...
            <CardDescription className='text-slate-400'>
              Rate per distance unit by band. Leave &quot;Distance To&quot;
            ...
            <Table>
              <TableHeader>
                <TableRow className='border-slate-700 bg-slate-700/50'>
                  ...
                  <TableHead className='text-slate-300'>
                    Distance To (empty = open-ended)
                  </TableHead>
                  <TableHead className='text-slate-300 bg-slate-700/70'>
                    Agency Rate
                  </TableHead>
                  {driverClasses.length > 0 ? (
                    driverClasses.map((dc) => (
              ...
              <TableBody>
                {distanceBands.map((band, i) => (
                  <TableRow key={i} className='border-slate-700'>
                    ...
                    <TableCell className='text-white bg-slate-700/30'>
                      <Input
                        type='number'
                        min={0}
                        step='0.01'
              ...
              <TableBody>
                {additionalCharges.length === 0 ? (
                  <TableRow className='border-slate-700'>
                    <TableCell
                      colSpan={
                        driverClasses.length > 0 ? 5 + driverClasses.length : 6
                      }
                  ...
                  additionalCharges.map((charge, i) => (
                    <TableRow key={i} className='border-slate-700'>
                        ...
                        driverClasses.map((dc) => (
                          <TableCell key={dc.id}>
                            <Input
                              ...
                              step='0.01'
                              value={
                                charge.driver_rates_by_class?.[dc.code] ?? ''
                              }
                              onChange={(e) =>
                                updateChargeDriverRateForClass(
                                  i,
                                  dc.code,
                                  Number(e.target.value) || 0,
                                )
                      ...
                      <TableCell>
                        ...
                        >
                          <SelectTrigger className='text-white bg-slate-700 border-slate-600 text-white h-9'>
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent className='bg-slate-800 border-slate-700'>
                            {CHARGE_UNITS.map((u) => (
            ...
            <div className='flex-1 space-y-2'>
              ...
              <Select
                ...
                onValueChange={(id) => {
                  ...
                  const card = existingRateCards.find(
                    (c) => String(c.id) === id,
                  );
                ...
                <SelectContent className='bg-slate-800 border-slate-700'>
                  {existingRateCards.map((c) => (
                    <SelectItem key={c.id} value={String(c.id)}>
                      {c.name} ({c.effective_from.slice(0, 10)} –{' '}
                      {c.effective_to.slice(0, 10)})
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
      ...
      <Card className='bg-slate-800 border-slate-700'>
        <CardHeader>
          ...
          <CardDescription className='text-slate-400'>
            Save as draft to continue later, or activate to make this rate card
            effective
          </CardDescription>
        </CardHeader>
