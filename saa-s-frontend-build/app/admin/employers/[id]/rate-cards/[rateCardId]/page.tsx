...
export default function ViewRateCardPage() {
  ...
  useEffect(() => {
    if (employerId && rateCardId) {
      fetchRateCard();
      apiClient
        .getEmployer(Number(employerId))
        .then((e) => setEmployerName(e.name))
        .catch(() => {});
    }
  ...
  const fetchRateCard = async () => {
    ...
    setError('');
    try {
      const data = await apiClient.getRateCard(
        Number(employerId),
        Number(rateCardId),
      );
      setRateCard(data);
        ...
        <div className='flex items-center gap-4'>
          ...
          <div>
            ...
            <p className='text-slate-400 mt-1'>
              {employerName
                ? `Rate card for ${employerName}`
                : 'Rate card details'}
            </p>
          </div>
        </div>
        <Link
          href={`/admin/employers/${employerId}/rate-cards/${rateCardId}/edit`}
        >
          ...
          {rateCard.rates &&
            (() => {
              ...
              const hasStructured =
                ...
                (r.minimum_trip_pay_driver_by_class &&
                  Object.keys(r.minimum_trip_pay_driver_by_class).length > 0) ||
                (r.distance_bands && r.distance_bands.length > 0) ||
                (r.additional_charges && r.additional_charges.length > 0);
              if (!hasStructured && Object.keys(rateCard.rates).length === 0)
                return null;
              return (
                <>
                  ...
                  {(r.minimum_trip_pay_agency != null ||
                    r.minimum_trip_pay_driver != null ||
                    (r.minimum_trip_pay_driver_by_class &&
                      Object.keys(r.minimum_trip_pay_driver_by_class).length >
                        0)) && (
                  ...
                  {r.distance_bands &&
                    r.distance_bands.length > 0 &&
                    (() => {
                      const hasByClass = r.distance_bands.some(
                        (b) =>
                      ...
                      const classCodes = hasByClass
                        ? [
                            ...new Set(
                              r.distance_bands.flatMap((b) =>
                                Object.keys(b.driver_rates_by_class || {}),
                              ),
                            ),
                          ]
                        : [];
                  ...
                  {r.additional_charges &&
                    r.additional_charges.length > 0 &&
                    (() => {
                      const hasByClass = r.additional_charges!.some(
                        (c) =>
                      ...
                      const chargeClassCodes = hasByClass
                        ? [
                            ...new Set(
                              r.additional_charges!.flatMap((c) =>
                                Object.keys(c.driver_rates_by_class || {}),
                              ),
                            ),
                          ]
                        : [];
                  ...
                  {!r.distance_bands?.length &&
                    !r.additional_charges?.length &&
                    Object.keys(rateCard.rates).length > 0 && (
                      <div className='pt-2'>
                        <p className='text-slate-400 text-sm mb-2'>
                          Rates (raw)
                        </p>
                        <pre className='text-white bg-slate-700/50 rounded p-4 text-slate-300 text-sm overflow-x-auto'>
