...
export default function AdminTimesheetDetailPage() {
  ...
  const [actionLoading, setActionLoading] = useState<'approve' | 'reject' | 'paid' | null>(null);
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {canEdit && (
            <Button variant="outline" size="sm" onClick={handleRecalculate} disabled={saving} className="bg-slate-700 border-slate-600 text-white">
          ...
          {canReject && (
            <Button variant="destructive" size="sm" onClick={() => setRejectOpen(true)} disabled={!!actionLoading}>
              {actionLoading === 'reject' ? <Loader2 className="h-4 w-4 animate-spin" /> : <XCircle className="h-4 w-4 mr-1" />}
              Reject
            </Button>
          )}
          {canMarkPaid && (
            <Button size="sm" onClick={handleMarkPaid} disabled={!!actionLoading} className="bg-emerald-600 hover:bg-emerald-700">
              {actionLoading === 'paid' ? <Loader2 className="h-4 w-4 animate-spin" /> : <Banknote className="h-4 w-4 mr-1" />}
              Mark as paid
            </Button>
          )}
        </div>
      ...
      <Card className="bg-slate-800 border-slate-700">
        <CardHeader>
          ...
          <CardDescription className="text-slate-400">
            Driver: {timesheet.driver?.user?.name ?? timesheet.driver?.name ?? `#${timesheet.driver_id}`} — Weekly total:{' '}
            <span className="font-semibold text-white">${Number(weeklyTotal).toFixed(2)}</span>
          </CardDescription>
          {timesheet.reject_reason && (
            <Alert variant="destructive" className="mt-2">
              <AlertCircle className="h-4 w-4" />
              <AlertDescription>Rejection reason: {timesheet.reject_reason}</AlertDescription>
            </Alert>
          )}
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          {canEdit && (
            <div className="flex gap-2">
              <Button onClick={() => setAddTripOpen(true)} size="sm" className="bg-slate-700 hover:bg-slate-600">
          ...
          {trips.length === 0 ? (
            ...
            <div className="space-y-6">
              {sortedDates.map((dateStr) => (
                <div key={dateStr}>
                  <h3 className="text-sm font-medium text-slate-400 mb-2">
                    ...
                    {dailyTotals.find((d) => d.date === dateStr)?.total.toFixed(2) ?? '0.00'}
                  </h3>
              ...
              <div className="border-t border-slate-700 pt-4 flex justify-end">
                <p className="text-lg font-semibold text-white">Weekly total: ${Number(weeklyTotal).toFixed(2)}</p>
              </div>
            </div>
          )}
        </CardContent>
      ...
      <Dialog open={addTripOpen} onOpenChange={setAddTripOpen}>
        <DialogContent className="bg-slate-800 border-slate-700">
          <DialogHeader>
            ...
            <DialogDescription className="text-slate-400">Rates are calculated from the employer Rate Card. Enter trip details.</DialogDescription>
          </DialogHeader>
          <form onSubmit={handleAddTrip} className="space-y-4">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="space-y-2">
                ...
                <Input type="number" min={0} value={newTripStopsCount} onChange={(e) => setNewTripStopsCount(e.target.value)} className="bg-slate-700 border-slate-600 text-white" />
              </div>
              <div className="space-y-2">
                <Label className="text-slate-300">Delay (hrs)</Label>
                <Input type="number" min={0} step="0.01" value={newTripDelayHours} onChange={(e) => setNewTripDelayHours(e.target.value)} className="bg-slate-700 border-slate-600 text-white" />
              </div>
              <div className="space-y-2">
                <Label className="text-slate-300">Handbomb</Label>
                <Input type="number" min={0} value={newTripHandbombCount} onChange={(e) => setNewTripHandbombCount(e.target.value)} className="bg-slate-700 border-slate-600 text-white" />
              </div>
            </div>
            <div className="space-y-2">
              <Label className="text-slate-300">Notes (optional)</Label>
              <Input value={newTripNotes} onChange={(e) => setNewTripNotes(e.target.value)} placeholder="Notes" className="bg-slate-700 border-slate-600 text-white" />
            </div>
            <DialogFooter>
      ...
      <Dialog open={rejectOpen} onOpenChange={setRejectOpen}>
        <DialogContent className="bg-slate-800 border-slate-700">
          <DialogHeader>
            ...
            <DialogDescription className="text-slate-400">Optionally provide a reason for the driver.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
          ...
          <DialogFooter>
            <Button type="button" variant="outline" onClick={() => setRejectOpen(false)} className="border-slate-600 text-slate-300">
              Cancel
            </Button>
