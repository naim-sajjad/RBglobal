...
import { Timesheet, TimesheetStatus, DriverWithDetails, Employer } from '@/lib/types';
...
export default function AdminTimesheetsPage() {
  ...
  useEffect(() => {
    fetchTimesheets();
  }, [driverId, employerId, statusFilter]);

  const fetchTimesheets = async () => {
    setLoading(true);
    setError('');
    try {
      const params: { driver_id?: number; employer_id?: number; status?: string } = {};
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400 mt-1">View and manage driver timesheets; approve, reject, or mark as paid</p>
      </div>
      ...
      <Card className="bg-slate-800 border-slate-700">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            ...
            <Select value={statusFilter} onValueChange={setStatusFilter}>
              ...
              <SelectContent className="bg-slate-800 border-slate-700">
                ...
                {(['draft', 'submitted', 'under_review', 'approved', 'rejected', 'paid'] as TimesheetStatus[]).map(
                  (s) => (
        ...
        <CardContent>
          {loading ? (
            ...
            <p className="text-slate-400 py-12 text-center">No timesheets match the filters.</p>
          ) : (
            <Table>
              <TableHeader>
                ...
                {timesheets.map((ts) => (
                  <TableRow key={ts.id} className="border-slate-700 hover:bg-slate-700/50">
                    <TableCell className="text-white">
                      {ts.driver?.user?.name ?? ts.driver?.name ?? `Driver #${ts.driver_id}`}
                    </TableCell>
                    <TableCell className="text-slate-300">
                      {formatDate(ts.week_start_date)} – {formatDate(ts.week_end_date)}
                    </TableCell>
                    <TableCell>
                      <Badge className={STATUS_COLORS[ts.status]}>{ts.status.replace('_', ' ')}</Badge>
                    </TableCell>
