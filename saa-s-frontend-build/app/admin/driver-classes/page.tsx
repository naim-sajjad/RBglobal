...
import { Plus, AlertCircle, Pencil, Trash2, MoreHorizontal } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export default function DriverClassesPage() {
  ...
  const handleDelete = async (dc: DriverClass) => {
    if (!confirm(`Delete driver class "${dc.code}"${dc.name ? ` (${dc.name})` : ''}?`)) return;
    try {
  ...
  const getStatusBadge = (status: string) => (
    <Badge className={status === 'active' ? 'bg-green-600' : 'bg-slate-600'}>{status}</Badge>
  );
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400 mt-1">Configure pay tiers (e.g. 21, 22, 23) and assign to drivers</p>
      </div>
                ...
                {classes.length === 0 ? (
                  <TableRow className="border-slate-700">
                    <TableCell colSpan={5} className="text-slate-400 text-center py-8">
                      No driver classes. Create one to use in rate cards and driver assignment.
                    </TableCell>
                  </TableRow>
                ) : (
                  classes.map((dc) => (
                    <TableRow key={dc.id} className="border-slate-700 hover:bg-slate-700/50">
                      <TableCell className="font-medium text-white">{dc.code}</TableCell>
                      <TableCell className="text-slate-300">{dc.name || '—'}</TableCell>
                      <TableCell className="text-slate-300 max-w-xs truncate">{dc.description || '—'}</TableCell>
