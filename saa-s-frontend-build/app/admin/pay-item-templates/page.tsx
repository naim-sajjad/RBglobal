...
const UNIT_OPTIONS = ['per_km', 'per_mile', 'per_hour', 'per_stop', 'flat', 'other'];
...
export default function PayItemTemplatesPage() {
  ...
  const [form, setForm] = useState({ code: '', name: '', unit: 'per_km', is_active: true });
  ...
  const fetchTemplates = async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to load pay item templates');
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-slate-400 mt-1">Define pay item types (e.g. Distance, Delay, Stops) used in driver timesheets. Set per-employer rates on each employer page.</p>
      </div>
      ...
      <Card className="bg-slate-800 border-slate-700">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
          <span className="text-slate-400 text-sm">{templates.length} template(s)</span>
        ...
        <CardContent>
          {loading ? (
            ...
            <p className="text-slate-400 py-8 text-center">No pay item templates. Create one to use in timesheets.</p>
          ) : (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {templates.map((t) => (
                  <TableRow key={t.id} className="border-slate-700">
                    <TableCell className="text-white font-mono">{t.code}</TableCell>
                    <TableCell className="text-white">{t.name}</TableCell>
                    <TableCell className="text-slate-300">{t.unit}</TableCell>
                    <TableCell>
                      <Badge className={t.is_active ? 'bg-green-600' : 'bg-slate-600'}>
                    ...
                    <TableCell>
                      <div className="flex gap-1">
                        ...
                        <Button variant="ghost" size="icon" className="h-8 w-8 text-red-400" onClick={() => handleDelete(t)} title="Delete">
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="bg-slate-800 border-slate-700">
          <DialogHeader>
            <DialogTitle className="text-white">{editing ? 'Edit' : 'New'} pay item template</DialogTitle>
            <DialogDescription className="text-slate-400">Code must be unique per tenant (e.g. distance_km, delay_hr).</DialogDescription>
          </DialogHeader>
