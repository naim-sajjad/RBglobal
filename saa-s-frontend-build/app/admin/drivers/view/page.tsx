...
export default function DriverDetailPage() {
  ...
  const handleDriverClassChange = async (driverClassId: string) => {
    if (!driver) return;
    const value = driverClassId === '__none__' ? null : parseInt(driverClassId, 10);
    if (value !== null && isNaN(value)) return;
    setUpdatingDriverClass(true);
    try {
      await apiClient.updateDriver(driver.id, {
        driver_class_id: value,
        driver_class_effective_date: driver.driver_class_effective_date || undefined,
      });
      toast.success('Driver class updated. Timesheet rates use this class for payroll.');
      await fetchDriver();
    } catch (err: any) {
      toast.error(err.response?.data?.message || 'Failed to update driver class');
    } finally {
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      try {
        ...
        referenceChecks = Array.isArray(checksData) ? checksData : checksData?.data ?? [];
      } catch {
        referenceChecks = [];
      }
      ...
      const addFormField = (label: string, value: string, x: number, y: number, labelWidth: number = 50, fieldWidth: number = 100) => {
        doc.setFontSize(10);
      ...
      const addCheckbox = (label: string, checked: boolean, x: number, y: number) => {
        doc.setFontSize(10);
        doc.setFont('helvetica', 'normal');
      ...
      try {
        ...
        const logoSrc = typeof RbLogo === 'string' 
          ? RbLogo 
          : (RbLogo as any)?.src || (RbLogo as any)?.default || String(RbLogo);
...
const xCentered = (pageWidth - logoWidth) / 2;
        ...
        doc.addImage(logoDataUrl, 'JPEG', xCentered, yPos, logoWidth, logoHeight);
      ...
      const middleInitial = parsedComplianceData?.personal?.middle_initial || '';
      ...
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      yPos = addFormField('Current Address', parsedComplianceData?.address?.current_address || '', margin, yPos, 45, 120);
      yPos = addFormField('City', parsedComplianceData?.address?.city || '', margin, yPos, 25, 60);
      ...
      drawLine(margin + 210, yPos - lineHeight + 2, Math.max(60, postalCodeTextWidth + 10));
      yPos += lineHeight;
      
      yPos = addFormField('Cell #', parsedComplianceData?.address?.cell_phone || '', margin, yPos, 25, 80);
      yPos += 5;
      ...
      if (parsedComplianceData?.address?.previous_addresses?.length > 0) {
        ...
        doc.setFontSize(10);
        doc.setFont('helvetica', 'normal');
        parsedComplianceData.address.previous_addresses.forEach((addr: any, index: number) => {
          checkNewPage(15);
          yPos = addFormField(`Address ${index + 1}`, addr.address || '', margin, yPos, 35, 120);
          yPos = addFormField('How long', addr.duration || '', margin + 80, yPos - lineHeight, 25, 60);
          yPos += 2;
        });
        yPos += 3;
      }
      ...
      const workEligible = parsedComplianceData?.personal?.work_eligibility_canada?.toLowerCase() === 'yes';
      ...
      const dob = parsedComplianceData?.personal?.date_of_birth 
        ? new Date(parsedComplianceData.personal.date_of_birth).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })
        : '';
      yPos = addFormField('Date of Birth (D/M/Y)', dob, margin, yPos, 50, 60);
      
      // Education
      if (parsedComplianceData?.personal?.education) {
        yPos = addFormField('Certification or Education', parsedComplianceData.personal.education, margin, yPos, 60, 100);
      }
      
      // Medical Limitations
      const medicalQuestion = 'Do you have any physical difficulties or medical limitation that might stop you from performing the position of a truck driver:';
      ...
      const hasMedicalLimits = parsedComplianceData?.personal?.medical_limitations?.toLowerCase() === 'yes';
      addCheckbox('YES', hasMedicalLimits, margin + 50, yPos);
      addCheckbox('NO', !hasMedicalLimits, margin + 90, yPos);
      yPos += lineHeight + 3;
      
      if (hasMedicalLimits && parsedComplianceData?.personal?.medical_limitations_explanation) {
        doc.text('If Yes, please explain: -', margin, yPos);
        yPos += lineHeight;
        doc.text(parsedComplianceData.personal.medical_limitations_explanation, margin, yPos);
      ...
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      yPos = addFormField('Licence Number', driver.license_number || '', margin, yPos, 40, 100);
      yPos = addFormField('Province', parsedComplianceData?.license?.license_province || '', margin, yPos, 25, 30);
      yPos = addFormField('Class', parsedComplianceData?.license?.license_class || '', margin + 80, yPos - lineHeight, 20, 30);
      const expiryDate = driver.license_expiry_date 
        ? new Date(driver.license_expiry_date).toLocaleDateString('en-CA').replace(/\//g, '-')
        : '';
      yPos = addFormField('Expiry', expiryDate, margin + 130, yPos - lineHeight, 20, 40);
      yPos += 2;
      
      if (parsedComplianceData?.license?.license_endorsements) {
        yPos = addFormField('Endorsements', parsedComplianceData.license.license_endorsements, margin, yPos, 35, 100);
      }
      if (parsedComplianceData?.license?.license_conditions) {
        yPos = addFormField('Conditions', parsedComplianceData.license.license_conditions, margin, yPos, 30, 100);
      }
      yPos += 3;
      
      // License Questions
      doc.text('Have you ever been denied a license or permit to operate a vehicle?', margin, yPos);
      const licenseDenied = parsedComplianceData?.questions?.license_denied?.toLowerCase() === 'yes';
      addCheckbox('Yes', licenseDenied, margin + 110, yPos);
      addCheckbox('No', !licenseDenied, margin + 150, yPos);
      yPos += lineHeight + 3;
      
      doc.text('Have you ever had your driving privileges revoked or suspended?', margin, yPos);
      const privilegesRevoked = parsedComplianceData?.questions?.privileges_revoked?.toLowerCase() === 'yes';
      ...
      const hasDangerousGoods = parsedComplianceData?.questions?.dangerous_goods_certificate?.toLowerCase() === 'yes';
      ...
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      yPos = addFormField('Years of Experience', (driver.years_of_experience || 0).toString(), margin, yPos, 50, 40);
      yPos += 5;
      
      // Equipment Used During the Last 5 Years
      if (parsedComplianceData?.driving_experience?.equipment_used?.length > 0) {
        checkNewPage(40);
        doc.setFontSize(10);
        doc.setFont('helvetica', 'normal');
        doc.text('List the types of equipment used during the last 5 years:', margin, yPos);
        yPos += lineHeight + 3;
        ...
        const colStarts = [margin, margin + 40, margin + 80, margin + 120, margin + 175];
        ...
        doc.setFontSize(9);
        doc.setFont('helvetica', 'normal');
        parsedComplianceData.driving_experience.equipment_used.forEach((equip: any) => {
          checkNewPage(10);
      ...
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      const accidentData = parsedComplianceData?.driving_experience?.accident_history;
      doc.text('Ever had accidents:', margin, yPos);
      const hasAccidents = accidentData?.ever_had_accidents?.toLowerCase() === 'yes';
      addCheckbox('Yes', hasAccidents, margin + 50, yPos);
      addCheckbox('No', !hasAccidents, margin + 90, yPos);
      yPos += lineHeight + 2;
      
      if (hasAccidents) {
        yPos = addFormField('Number of incidents', accidentData?.number_of_incidents || '', margin, yPos, 50, 40);
        yPos += 2;
        doc.text('If Yes, please explain: -', margin, yPos);
        yPos += lineHeight;
      ...
      if (parsedComplianceData?.driving_experience?.traffic_violations?.length > 0) {
        checkNewPage(40);
        doc.setFontSize(10);
        doc.setFont('helvetica', 'bold');
        doc.text('Last 3 years history of traffic violations and convictions', margin, yPos);
        drawLine(margin, yPos + 2, maxWidth);
        yPos += lineHeight + 5;
        ...
        doc.setFontSize(9);
        doc.setFont('helvetica', 'normal');
        parsedComplianceData.driving_experience.traffic_violations.forEach((violation: any) => {
          checkNewPage(10);
      ...
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      const currentEmp = parsedComplianceData?.employment_history?.current_employer;
      if (currentEmp) {
        yPos = addFormField('Company', currentEmp.company || '', margin, yPos, 30, 120);
        yPos = addFormField('Supervisor', currentEmp.supervisor || '', margin, yPos, 30, 120);
        yPos = addFormField('Address', currentEmp.address || '', margin, yPos, 30, 120);
        yPos = addFormField('Phone', currentEmp.phone || '', margin, yPos, 25, 100);
        yPos = addFormField('Position', currentEmp.position || '', margin, yPos, 30, 120);
        yPos = addFormField('Start Date', currentEmp.start_date || '', margin, yPos, 30, 100);
        yPos = addFormField('End Date', currentEmp.end_date || '', margin, yPos, 30, 100);
      ...
      // Previous Employers
      if (parsedComplianceData?.employment_history?.previous_employers?.length > 0) {
        parsedComplianceData.employment_history.previous_employers.forEach((emp: any, index: number) => {
          checkNewPage(50);
          doc.setFontSize(10);
          doc.setFont('helvetica', 'bold');
          ...
          doc.setFontSize(10);
          doc.setFont('helvetica', 'normal');
          yPos = addFormField('Company', emp.company || '', margin, yPos, 30, 120);
          yPos = addFormField('Supervisor', emp.supervisor || '', margin, yPos, 30, 120);
          yPos = addFormField('Address', emp.address || '', margin, yPos, 30, 120);
          yPos = addFormField('Phone', emp.phone || '', margin, yPos, 25, 100);
          yPos = addFormField('Position', emp.position || '', margin, yPos, 30, 120);
          yPos = addFormField('Start Date', emp.start_date || '', margin, yPos, 30, 100);
          yPos = addFormField('End Date', emp.end_date || '', margin, yPos, 30, 100);
      ...
      if (referenceChecks.length > 0) {
        referenceChecks.forEach((refCheck: any, refIndex: number) => {
          ...
          doc.setFont('helvetica', 'bold');
          doc.setFontSize(11);
          doc.text(`Pre-Employment Reference Check ${refIndex + 1}`, margin, yPos);
          doc.setFont('helvetica', 'normal');
          doc.setFontSize(9);
          doc.text(`Status: ${refCheck.status || 'N/A'} | Filled by: ${refCheck.filled_by || 'N/A'}`, margin, yPos + 6);
          yPos += 12;

          const req = refCheck.reference_request || {};
          doc.setFont('helvetica', 'bold');
          doc.setFontSize(10);
          doc.text('Request for Information from Previous Employer', margin, yPos);
          yPos += 6;
          doc.setFont('helvetica', 'normal');
          doc.setFontSize(9);
          yPos = addFormField('Applicant', req.applicant_name || '', margin, yPos, 28, 80);
          yPos = addFormField('Driver\'s License', req.drivers_license_number || '', margin, yPos, 35, 70);
          yPos = addFormField('Previous Company', req.previous_company_name || '', margin, yPos, 38, 90);
          yPos = addFormField('Phone', req.previous_company_phone || '', margin, yPos, 20, 70);
          yPos = addFormField('Supervisor/Employer', req.supervisor_employer_name || '', margin, yPos, 45, 85);
          yPos += 4;
          ...
          if (formData) {
            ...
            doc.setFont('helvetica', 'normal');
            doc.setFontSize(9);
            yPos = addFormField('Date of Reference Check', formData.date_of_reference_check || '', margin, yPos, 50, 60);
            doc.text('Relationship:', margin, yPos);
            doc.text(formData.relationship_to_applicant === 'other' ? `Other: ${formData.relationship_other_specify || ''}` : 'Supervisor', margin + 50, yPos);
            yPos += lineHeight;
            yPos = addFormField('Employment From', formData.date_of_employment_from || '', margin, yPos, 38, 50);
            yPos = addFormField('To', formData.date_of_employment_to || '', margin + 95, yPos - lineHeight, 12, 50);
            yPos += 2;
            yPos = addFormField('Position(s) Held', formData.positions_held || '', margin, yPos, 38, 100);
            yPos = addFormField('Nature of Job', formData.nature_of_job || '', margin, yPos, 35, 110);
            doc.text('Driver off (illness/injury):', margin, yPos);
            doc.text(formData.driver_off_illness_injury || '', margin + 65, yPos);
            yPos += lineHeight;
            ...
            doc.text(String(formData.reason_for_leaving || '').replace(/_/g, ' '), margin + 45, yPos);
            yPos += lineHeight + 2;
            ...
            doc.text(formData.ability_to_follow_instructions_rating || '', margin + 145, yPos);
            yPos += lineHeight;
            doc.text('Quality of Work:', margin, yPos);
            doc.text(formData.quality_of_work_rating || '', margin + 40, yPos);
            yPos += lineHeight + 2;
            yPos = addFormField('Name of Person Supplying Info', formData.name_of_person_supplying_info || '', margin, yPos, 55, 90);
            yPos = addFormField('Date', formData.referee_signature_date || '', margin, yPos, 15, 50);
            if (formData.additional_comments) {
              yPos += 2;
              doc.text('Additional comments:', margin, yPos);
              yPos += lineHeight;
              const commentLines = doc.splitTextToSize(formData.additional_comments, maxWidth);
      ...
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(12);
      doc.text('To be read and signed by the applicant', pageWidth / 2, yPos, { align: 'center' });
yPos += 10;
...
const signedDate = new Date().toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' });
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="bg-slate-800 border-slate-700">
            <CardHeader>
              ...
              <CardDescription className="text-slate-400">
                Used to calculate driver pay from Employer Rate Cards. Timesheet rates are resolved by this class.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  {updatingDriverClass ? (
                    ...
                    <Select
                      value={driver.driver_class_id != null ? String(driver.driver_class_id) : '__none__'}
                ...
                <div>
                  ...
                  {driver.driver_class_effective_date && (
                    <p className="text-slate-400 text-xs mt-1">
                      Effective: {new Date(driver.driver_class_effective_date).toLocaleDateString()}
                    </p>
                  )}
                </div>
              </div>
              {!driver.driver_class_id && (
                <p className="text-amber-400 text-sm">
                  Assign a driver class so timesheet trips can calculate driver pay from employer rate cards.
                </p>
              )}
            </CardContent>
          </Card>
            ...
            <Card className="bg-slate-800 border-slate-700">
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <p className="text-slate-400 text-sm">Work Eligibility in Canada</p>
                    <p className="text-white font-medium capitalize">
                      {parsedComplianceData.personal.work_eligibility_canada || 'N/A'}
                    </p>
                  </div>
                  <div>
                    <p className="text-slate-400 text-sm">Medical Limitations</p>
                    <p className="text-white font-medium capitalize">
                      {parsedComplianceData.personal.medical_limitations || 'N/A'}
                    </p>
                  </div>
                </div>
                {parsedComplianceData.personal.medical_limitations === 'yes' && parsedComplianceData.personal.medical_limitations_explanation && (
                  <div>
                    <p className="text-slate-400 text-sm mb-2">Medical Limitations Explanation</p>
                    <p className="text-white text-sm bg-slate-700/50 p-3 rounded-lg">
          ...
          {parsedComplianceData?.address && (
            <Card className="bg-slate-800 border-slate-700">
              ...
              <CardContent className="space-y-4">
                <div>
                  <h4 className="text-white font-medium mb-3">Current Address</h4>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                ...
                {parsedComplianceData.address.previous_addresses?.length > 0 && (
                  <div className="pt-4 border-t border-slate-700">
                    <h4 className="text-white font-medium mb-3">Previous Addresses (Last 3 Years)</h4>
                    <div className="space-y-4">
                      {parsedComplianceData.address.previous_addresses.map((addr: any, index: number) => (
                        <div key={index} className="bg-slate-700/50 p-3 rounded-lg">
                          <p className="text-white font-medium mb-2">Address {index + 1}</p>
                          <p className="text-slate-300 text-sm">{addr.address || 'N/A'}</p>
                          <p className="text-slate-400 text-xs mt-1">Duration: {addr.duration || 'N/A'}</p>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-white font-medium">
                      {parsedComplianceData.license.license_endorsements || 'N/A'}
                    </p>
                  </div>
                  <div>
                  ...
                  <div className="pt-4 border-t border-slate-700">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="text-white font-medium capitalize">
                          {parsedComplianceData.questions.license_denied || 'N/A'}
                        </p>
                      </div>
                      <div>
                        <p className="text-slate-400 text-sm">Privileges Revoked</p>
                        <p className="text-white font-medium capitalize">
                          {parsedComplianceData.questions.privileges_revoked || 'N/A'}
                        </p>
                      </div>
                      <div>
                        <p className="text-slate-400 text-sm">Dangerous Goods Certificate</p>
                        <p className="text-white font-medium capitalize">
                          {parsedComplianceData.questions.dangerous_goods_certificate || 'N/A'}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-slate-700">
                        ...
                        <th className="text-left text-slate-300 p-2">Tractor Type</th>
                        <th className="text-left text-slate-300 p-2">Transmissions</th>
                        <th className="text-left text-slate-300 p-2">Trailer Type</th>
                        <th className="text-left text-slate-300 p-2">Areas Operated</th>
                      </tr>
                    </thead>
                    <tbody>
                      {parsedComplianceData.driving_experience.equipment_used.map((equip: any, index: number) => (
                        <tr key={index} className="border-b border-slate-700/50">
                          <td className="text-white p-2">{equip.make || 'N/A'}</td>
                          <td className="text-white p-2">{equip.tractor_type || 'N/A'}</td>
                          <td className="text-white p-2">{equip.transmissions || 'N/A'}</td>
                          <td className="text-white p-2">{equip.trailer_type || 'N/A'}</td>
                          <td className="text-white p-2">{equip.areas_operated || 'N/A'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-white font-medium capitalize">
                      {parsedComplianceData.driving_experience.accident_history.ever_had_accidents || 'N/A'}
                    </p>
                  </div>
                  {parsedComplianceData.driving_experience.accident_history.ever_had_accidents === 'yes' && (
                    <>
                      <div>
                        <p className="text-slate-400 text-sm">Number of Incidents</p>
                        <p className="text-white font-medium">
                          {parsedComplianceData.driving_experience.accident_history.number_of_incidents || 'N/A'}
                        </p>
                      </div>
                      {parsedComplianceData.driving_experience.accident_history.accident_explanation && (
                        <div className="col-span-2">
                          <p className="text-slate-400 text-sm mb-2">Explanation</p>
                          <p className="text-white text-sm bg-slate-700/50 p-3 rounded-lg">
                            {parsedComplianceData.driving_experience.accident_history.accident_explanation}
                          </p>
                        </div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-slate-700">
                        ...
                        <th className="text-left text-slate-300 p-2">Location</th>
                        <th className="text-left text-slate-300 p-2">Violation/Charge</th>
                        <th className="text-left text-slate-300 p-2">Penalty</th>
                      </tr>
                    </thead>
                    <tbody>
                      {parsedComplianceData.driving_experience.traffic_violations.map((violation: any, index: number) => (
                        <tr key={index} className="border-b border-slate-700/50">
                          <td className="text-white p-2">{violation.date || 'N/A'}</td>
                          <td className="text-white p-2">{violation.location || 'N/A'}</td>
                          <td className="text-white p-2">{violation.violation_charge || 'N/A'}</td>
                          <td className="text-white p-2">{violation.penalty || 'N/A'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <Card className="bg-slate-800 border-slate-700">
              ...
              <CardContent className="space-y-6">
                ...
                {parsedComplianceData.employment_history.current_employer && (
                  <div>
                    <h4 className="text-white font-medium mb-4">Current/Most Recent Employer</h4>
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <p className="text-slate-400 text-sm">Company</p>
                        <p className="text-white font-medium">
                          {parsedComplianceData.employment_history.current_employer.company || 'N/A'}
                        </p>
                      </div>
                      <div>
                        <p className="text-slate-400 text-sm">Supervisor</p>
                        <p className="text-white font-medium">
                          {parsedComplianceData.employment_history.current_employer.supervisor || 'N/A'}
                        </p>
                      </div>
                      <div>
                        <p className="text-slate-400 text-sm">Address</p>
                        <p className="text-white font-medium">
                          {parsedComplianceData.employment_history.current_employer.address || 'N/A'}
                        </p>
                      </div>
                      <div>
                        <p className="text-slate-400 text-sm">Phone</p>
                        <p className="text-white font-medium">
                          {parsedComplianceData.employment_history.current_employer.phone || 'N/A'}
                        </p>
                      </div>
                      <div>
                        <p className="text-slate-400 text-sm">Position</p>
                        <p className="text-white font-medium">
                          {parsedComplianceData.employment_history.current_employer.position || 'N/A'}
                        </p>
                      </div>
                      <div>
                        <p className="text-slate-400 text-sm">Start Date</p>
                        <p className="text-white font-medium">
                          {parsedComplianceData.employment_history.current_employer.start_date || 'N/A'}
                        </p>
                      </div>
                      <div>
                        <p className="text-slate-400 text-sm">End Date</p>
                        <p className="text-white font-medium">
                          {parsedComplianceData.employment_history.current_employer.end_date || 'N/A'}
                        </p>
                      </div>
                      {parsedComplianceData.employment_history.current_employer.reasons_for_leaving && (
                        <div className="col-span-2">
                          <p className="text-slate-400 text-sm mb-2">Reasons for Leaving</p>
                          <p className="text-white text-sm bg-slate-700/50 p-3 rounded-lg">
                            {parsedComplianceData.employment_history.current_employer.reasons_for_leaving}
                          </p>
                        </div>
                ...
                {parsedComplianceData.employment_history.previous_employers?.length > 0 && (
                  <div className="pt-4 border-t border-slate-700">
                    <h4 className="text-white font-medium mb-4">Previous Employers (Last 10 Years)</h4>
                    <div className="space-y-6">
                      {parsedComplianceData.employment_history.previous_employers.map((emp: any, index: number) => (
                        <div key={index} className="bg-slate-700/50 p-4 rounded-lg">
                          <h5 className="text-white font-medium mb-3">Previous Employer {index + 1}</h5>
                          <div className="grid grid-cols-2 gap-4">
                            <div>
                              <p className="text-slate-400 text-sm">Company</p>
                              <p className="text-white font-medium">{emp.company || 'N/A'}</p>
                            </div>
                            <div>
                              <p className="text-slate-400 text-sm">Supervisor</p>
                              <p className="text-white font-medium">{emp.supervisor || 'N/A'}</p>
                            </div>
                            <div>
                              <p className="text-slate-400 text-sm">Address</p>
                              <p className="text-white font-medium">{emp.address || 'N/A'}</p>
                            </div>
                            <div>
                              <p className="text-slate-400 text-sm">Phone</p>
                              <p className="text-white font-medium">{emp.phone || 'N/A'}</p>
                            </div>
                            <div>
                              <p className="text-slate-400 text-sm">Position</p>
                              <p className="text-white font-medium">{emp.position || 'N/A'}</p>
                            </div>
                            <div>
                              <p className="text-slate-400 text-sm">Start Date</p>
                              <p className="text-white font-medium">{emp.start_date || 'N/A'}</p>
                            </div>
                            <div>
                              <p className="text-slate-400 text-sm">End Date</p>
                              <p className="text-white font-medium">{emp.end_date || 'N/A'}</p>
                            </div>
          ...
          <Card className="bg-slate-800 border-slate-700">
            ...
            <CardContent className="space-y-2">
              <Button
                ...
                onClick={() => router.push(`/admin/drivers/create?id=${driver.id}`)}
              >
